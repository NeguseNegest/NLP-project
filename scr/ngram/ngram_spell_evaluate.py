...
def normalize_lambdas(raw_lambdas, max_n_gram):
    ...
    if any(value < 0 for value in lambdas.values()):
...
def evaluate_dataset_strategy(corrector, examples, dataset_name, strategy_name, start_chars_typed):
    ...
    progress_label = f"N-gram spell {dataset_name} {strategy_name}"
...
def print_run_summary(result):
    print()
    print("=" * 72)
    print(f"N-gram spell correction — {result['dataset']} — {result['strategy'].upper()}")
    print("=" * 72)
...
def plot_run(result, plot_dir):
    ...
    prefix = f"ngram_spell_{dataset}_{strategy}"
...
def plot_dataset_comparison(dataset_name, results, plot_dir):
    ...
    output_path = plot_dir / f"ngram_spell_{dataset_name}_top{MAIN_TOP_K}_strategy_comparison.png"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    plt.ylim(0, 1)
    plt.ylabel("Score")
    plt.title(f"{dataset_name} - N-gram spell strategy comparison")
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_path, dpi=160)
    plt.close()
...
def save_run_outputs(result, metrics_dir):
    ...
    result_path = metrics_dir / f"ngram_spell_{dataset}_{strategy}_results.json"
    examples_path = metrics_dir / f"ngram_spell_{dataset}_{strategy}_examples.jsonl"
...
def write_summary_outputs(all_results, metrics_dir):
    ...
    summary_json_path = metrics_dir / "ngram_spell_summary.json"
    ...
    summary_csv_path = metrics_dir / "ngram_spell_summary.csv"
    ...
    breakdown_csv_path = metrics_dir / "ngram_spell_breakdowns.csv"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate N-gram spell correction on fixed misspelling datasets."
    )
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument("--lambda_path", default=None)
    parser.add_argument(
        "--lambdas",
        default=None,
        help="Comma-separated override, for example 0,0,0.1,0.9",
    )
...
def build_predictor(args, dataset_name):
    ...
    model_path = (
        ...
        if args.model_path
        else resolve_existing_path(project_root, config["model_candidates"])
    )

    print()
    print(f"Loading N-gram model for {dataset_name}")
    print(f"Model path : {model_path}")
    model = NGramModel.load(model_path)

    if args.lambdas:
        lambdas = parse_lambdas_string(args.lambdas, max_n_gram=model.max_n_gram)
        lambda_source = "command_line"
    else:
        lambda_path = (
            Path(args.lambda_path)
            if args.lambda_path
            else resolve_existing_path(project_root, config["lambda_candidates"])
        )
        lambdas = load_lambdas(lambda_path, max_n_gram=model.max_n_gram)
        lambda_source = str(lambda_path)

    predictor = NGramSpellPredictor(model, lambdas)

    print(f"Word vocabulary size: {len(predictor.word_vocab):,}")
    print(f"Lambdas source      : {lambda_source}")
    print(f"Lambdas             : {lambdas}")

    return predictor, model_path, lambda_source, lambdas
...
def main():
    ...
    metrics_dir = (
        ...
        if args.metrics_dir
        else project_root / "results/metrics/ngram_spell"
    )
    plot_dir = (
        Path(args.plot_dir)
        if args.plot_dir
        else project_root / "results/plots/ngram_spell"
    )
    ...
    for dataset_name in datasets:
        predictor, model_path, lambda_source, lambdas = build_predictor(args, dataset_name)
        ...
        for strategy_name in strategies:
            ...
            result["configuration"] = {
                "model": "ngram",
                "ngram_model_path": str(model_path),
